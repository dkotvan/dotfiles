...
function MyPreviewer:populate_preview_buf(entry_str)
    if entry_str == nil then
        vim.notify("MyPreviewer:populate_preview_buf called with nil entry_str", vim.log.levels.INFO)
        return
    end

    local type, file = entry_str:match("^%[(%w+)%]%s+(.+)$")
    vim.notify("MyPreviewer:populate_preview_buf called " .. vim.inspect(entry_str), vim.log.levels.INFO)
    ...
    if type == "Git" then
        ...
        if handle then
            ...
            if status_line then
                ...
                vim.notify("MyPreviewer:populate_preview_buf called for Git with status " .. tostring(status) .. " and cmd  " .. git_diff_cmd, vim.log.levels.INFO)
            end
        end
        if git_diff_cmd ~= "" then
            vim.notify("MyPreviewer:populate_preview_buf called for Git with cmd  " .. git_diff_cmd, vim.log.levels.INFO)
        ...
        vim.notify("MyPreviewer:populate_preview_buf called for File " .. vim.inspect(file), vim.log.levels.INFO)
...
local function files_picker()
    vim.notify("files_picker CALLED", vim.log.levels.INFO)
    vim.notify("files_picker called (regular print)", vim.log.levels.INFO)
    -- Create a table to track added files to prevent duplicates
    local added_files = {}

    vim.notify("  files_picker - Calling fzf.fzf_exec", vim.log.levels.INFO)
    vim.notify("files_picker - Calling fzf.fzf_exec...", vim.log.levels.INFO)
    fzf.fzf_exec(function(fzf_cb)
        vim.notify("    files_picker.fzf_exec_callback - Populating Git files", vim.log.levels.INFO)
        ...
        vim.notify("    files_picker.fzf_exec_callback - Populating Buffers", vim.log.levels.INFO)
        ...
        vim.notify("    files_picker.fzf_exec_callback - Populating Files from fd/find", vim.log.levels.INFO)
        local file_cmd
        ...
        vim.notify("    files_picker.fzf_exec_callback - Signalling end of input", vim.log.levels.INFO)
        fzf_cb(nil)
    end, {
        prompt = "Files❯ ",
        ...
        actions = {
            ["default"] = function(selected)
                vim.notify("files_picker.actions.default called. Selection: " .. vim.inspect(selected),
                    vim.log.levels.INFO)
                vim.notify("files_picker.actions.default called (regular print)", vim.log.levels.INFO)
                if not selected or not selected[1] then
                    vim.notify("  files_picker.actions.default - No selection or empty selection.", vim.log.levels.INFO)
                    return
                end

                local type, file = selected[1]:match("^%[(%w+)%]%s+(.+)$")
                vim.notify(string.format("  files_picker.actions.default - Parsed: type='%s', file='%s'",
                    tostring(type), tostring(file)), vim.log.levels.INFO)

                if not file then
                    vim.notify("  files_picker.actions.default - Could not parse file from selection.",
                        vim.log.levels.INFO)
                    return
                end

                if type == "File" or type == "Git" then
                    vim.notify(string.format("    files_picker.actions.default - Editing file: %s", file),
                        vim.log.levels.INFO)
                    vim.cmd("edit " .. vim.fn.fnameescape(file))
                elseif type == "Buf" then
                    vim.notify(string.format("    files_picker.actions.default - Switching to buffer: %s", file),
                        vim.log.levels.INFO)
                    vim.cmd("buffer " .. vim.fn.fnameescape(file))
                else
                    vim.notify(string.format("    files_picker.actions.default - Unknown type '%s'", tostring(type)),
                        vim.log.levels.INFO)
                end
            end
        }
    })
    vim.notify("files_picker - fzf.fzf_exec call finished (regular print)", vim.log.levels.INFO)
    vim.notify("files_picker - fzf.fzf_exec call finished.", vim.log.levels.INFO)
end
